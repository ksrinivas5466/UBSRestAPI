...
public class EventsController {
	...
	@ResponseBody
	public ResponseEntity<List<Event>> getAllEventsByRepoIdAndActorId(@PathVariable Long repoId, @PathVariable Long actorId) {
		List<Event> events = eventService.getAllEventsByRepoIdAndActorId(repoId,actorId);
		if(!events.isEmpty()) { 
			return new ResponseEntity<List<Event>>(events, HttpStatus.OK);
		} else { 
			return new ResponseEntity<List<Event>>(HttpStatus.NOT_FOUND);
